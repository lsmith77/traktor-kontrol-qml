...
Before you change anything, copy Traktor’s `qml` folder somewhere safe.
...
If the mod overlay is a folder that contains `CSI`, `Defines`, `Screens` (etc.), you can merge it into the target `qml` like this:
...
If Traktor fails to start or screens break, restore:
